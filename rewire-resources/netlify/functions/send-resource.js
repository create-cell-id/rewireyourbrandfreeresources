...
exports.handler = async (event) => {
  ...
  let email, resource;
  try {
    ({ email, resource } = JSON.parse(event.body));
  } catch {
    return { statusCode: 400, headers, body: JSON.stringify({ error: 'Invalid request body' }) };
  }

  if (!email || !resource) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Missing email or resource' }) };

  const meta = RESOURCES[resource];
  if (!meta) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Unknown resource' }) };

  // PDFs live in public/pdfs — Netlify functions can read from the project root
  ...
  try {
    ...
    return { statusCode: 500, headers, body: JSON.stringify({ error: 'Resource file not found' }) };
  }
  ...
  try {
    await resend.emails.send({
      ...
      html: emailHTML(meta),
      attachments: [{ filename: meta.file, content: pdfBuffer }],
    });
    ...
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;">
        ...
        <tr><td style="background:#0D1326;padding:36px 36px 28px;border-radius:0 0 12px 12px;border:1px solid #1A2442;border-top:none;">
          ...
          <p style="margin:0 0 24px;font-size:15px;color:#8C9BB4;line-height:1.7;">It's attached to this email as a PDF. Open it, use it, and if you have questions — just reply here.</p>
          <table cellpadding="0" cellspacing="0" style="background:#111827;border-radius:10px;border:1px solid #1A2442;padding:20px 24px;width:100%;margin-bottom:28px;">
            <tr><td>
              ...
              <p style="margin:0;font-size:14px;color:#8C9BB4;line-height:1.65;">Book a free 45-minute Brand Strategy Call. I'll audit your brand, identify the gaps, and leave you with a clear direction.</p>
            </td></tr>
          </table>
          <table cellpadding="0" cellspacing="0" style="width:100%;margin-bottom:32px;">
            <tr><td align="center">
              <a href="https://api.leadconnectorhq.com/widget/bookings/rewireyourbrand" style="display:inline-block;background:#3B82F6;color:#fff;font-size:14px;font-weight:600;text-decoration:none;padding:13px 28px;border-radius:8px;">Book a Free Strategy Call →</a>
            </td></tr>
          </table>
